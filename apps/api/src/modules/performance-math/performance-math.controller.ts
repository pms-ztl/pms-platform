  ...
  async getCPIS(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const tenureYears = user.hireDate
        ? (now.getTime() - new Date(user.hireDate).getTime()) / (365.25 * 24 * 60 * 60 * 1000)
        : 1;
      ...
      const goalInputs: CPISInput['goals'] = goals.map(g => {
        ...
        if (g.dueDate) {
          if (g.completedAt) {
            daysLate = (g.completedAt.getTime() - g.dueDate.getTime()) / (1000 * 60 * 60 * 24);
          } else if (g.status !== 'COMPLETED' && now > g.dueDate) {
            daysLate = (now.getTime() - g.dueDate.getTime()) / (1000 * 60 * 60 * 24);
          }
        }
      ...
      if (allUpdates.length > 0) {
        ...
        for (const d of sorted) {
          const gap = (lastDate.getTime() - d.getTime()) / (1000 * 60 * 60 * 24);
